...
// (FINAL) – Ayarlar + Özel Hatırlatma Programı (toggle) + Minimal Tema Rengi seçici
...
class _CustomReminderScreenState extends State<CustomReminderScreen> {
  ...
  void _load() {
    _items = preferenceService.getReminderTimes();
    setState(() {});
  }

  Future<void> _rescheduleSilently() async {
    try {
      final startHour = preferenceService.getStartHour();
      final endHour = preferenceService.getEndHour();
      final interval =
      Duration(minutes: preferenceService.getReminderInterval());
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Bildirimler güncellenemedi. İzinleri kontrol edin.')),
      );
    }
  }

  // Saat ekleme: premium bottom sheet (CupertinoDatePicker, 24s)
  Future<void> _showAddTimeSheet() async {
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: CupertinoDatePicker(
                    ...
                    use24hFormat: true,
                    initialDateTime: DateTime(2024, 1, 1, selectedHour, selectedMinute),
                ...
                Padding(
                  ...
                  child: SizedBox(
                    ...
                    child: FilledButton.icon(
                      ...
                      onPressed: () async {
                        final s = '${selectedHour.toString().padLeft(2, '0')}:${selectedMinute.toString().padLeft(2, '0')}';

                        if (_items.any((e) => e.time == s)) {
                          if (!mounted) return;
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text('$s zaten listede.')),
                          );
                          return;
                        }
                        ...
                        // Optimistic add
                        setState(() {
                          _items.add(ReminderTime(time: s, enabled: true));
                          _items.sort((a, b) => a.time.compareTo(b.time));
                        });

                        // Save
                        try {
                          await preferenceService.saveReminderTimes(_items);
                          if (mounted) {
                            Navigator.pop(context); // alt sayfayı kapat
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Text('$s eklendi.')),
                            );
                          }
                          await _rescheduleSilently();
                        } catch (e) {
                          if (!mounted) return;
                          setState(() => _items.removeWhere((e) => e.time == s));
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('Saat kaydedilemedi.')),
                          );
                        }
                      },
                    ),
  ...
  Future<void> _toggle(ReminderTime item, bool value) async {
    // Optimistic toggle
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Değişiklik kaydedilemedi.')),
        );
      }
    }
  }

  Future<void> _delete(ReminderTime item) async {
    // Optimistic remove
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Saat silinemedi.')),
        );
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Özel Hatırlatma Programı'),
        backgroundColor: Colors.transparent,
        elevation: 0,
      ),
      floatingActionButton: FloatingActionButton(
        onPressed: _showAddTimeSheet,
      ...
      body: _items.isEmpty
        ...
        child: Padding(
          ...
          child: Text(
            'Henüz özel hatırlatma saati eklenmedi.\n\nSağ alttaki + ile yeni saat ekleyin.',
        ...
        itemBuilder: (context, i) {
          ...
          return Card(
            ...
            child: ListTile(
              leading: Icon(
                Icons.alarm,
                color: item.enabled ? primary : Colors.grey,
              ),
              title: Text(
                item.time,
                style: TextStyle(
              ...
              onTap: () => _toggle(item, !item.enabled), // tek dokunuş
            ),
          );
        },
      ),
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: <Widget>[
          ...
          // 1) Hatırlatma Programı
          _buildSettingsCard(
            title: 'Hatırlatma Programı',
            children: [
              ListTile(
                leading: Icon(Icons.schedule, color: primary),
                title: const Text('Özel Saatler Belirle'),
                subtitle: const Text('Kendi belirlediğiniz saatlerde bildirim alın.'),
                trailing: const Icon(Icons.arrow_forward_ios, size: 16),
                onTap: () {
                  Navigator.of(context)
                      .push(MaterialPageRoute(builder: (context) => const CustomReminderScreen()))
